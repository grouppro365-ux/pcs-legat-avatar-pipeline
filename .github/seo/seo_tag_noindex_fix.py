...
class P(HTMLParser):
 def __init__(self):super().__init__(convert_charrefs=True);self.meta={};self.canonical=None
...
def req(url,headers=None):
 q=urllib.request.Request(url,headers={'User-Agent':'SEOTagNoindexFix/1.0','Cache-Control':'no-cache',**(headers or {})})
 try:
...
$name='rank-math-options-titles';$opt=get_option($name,[]);if(!is_array($opt)){$opt=[];}
$before=$opt['tax_post_tag_robots']??null;$opt['tax_post_tag_robots']=['noindex'];$opt['tax_post_tag_custom_robots']='on';
$updated=update_option($name,$opt,false);wp_cache_delete($name,'options');if(function_exists('wp_cache_flush'))wp_cache_flush();
do_action('rank_math/sitemap/flush_cache');do_action('litespeed_purge_all');if(function_exists('w3tc_flush_all'))w3tc_flush_all();
$after=get_option($name,[])['tax_post_tag_robots']??null;$deleted=@unlink(__FILE__);
...
?>'''
def main():
 cfg=json.load(open(os.environ.get('SEO_PAYLOAD','/tmp/seo-payload.json'),encoding='utf-8'));b=cfg['beget'];host=cfg['ftp_host'];out={'mode':'tag_noindex_fix','sites':[]}
 for s in cfg['sites']:
  tok=hashlib.sha256(os.urandom(32)).hexdigest();name='.seo-tagfix-'+hashlib.sha256((s['domain']+tok).encode()).hexdigest()[:18]+'.php';code=php(s['domain'],tok)
  ...
  if ok:
   ...
   time.sleep(2)
   smst,smu,smb=req('https://'+s['domain']+'/sitemap_index.xml?_seo_tagfix='+str(int(time.time())));children=re.findall(r'<loc>\s*([^<]+?)\s*</loc>',smb,re.I);item['tag_sitemaps_after']=[x for x in children if 'post_tag-sitemap' in x]
   sample=s.get('sample_tag')
   if sample:
    pst,pu,pb=req(sample+'?_seo_tagfix='+str(int(time.time())));p=P();
    try:p.feed(pb)
    except:pass
    item['sample_after']={'status':pst,'robots':p.meta.get('robots'),'canonical':p.canonical}
