...
(function(root){
  ...
  function matchesDescriptor(d, spec = {}) {
    ...
    if (spec.name) {
      const hay = norm(d.name), needle = norm(spec.name);
      if (spec.exact ? hay !== needle : !hay.includes(needle)) return false;
    }
    if (spec.label) {
      const hay = norm(d.label), needle = norm(spec.label);
      if (spec.exact ? hay !== needle : !hay.includes(needle)) return false;
    }
    if (spec.text) {
      const hay = norm(d.text), needle = norm(spec.text);
      if (spec.exact ? hay !== needle : !hay.includes(needle)) return false;
    }
  ...
  class ElementProxy {
    ...
    target(extra = {}) {
      return {...this.descriptor,...extra,hints:{...(this.descriptor.hints||{}),...(extra.hints||{})}};
    }
    ...
    async val(value) { if(arguments.length===0)return this.readValue();return this.fill(value); }
  ...
  class Driver {
    ...
    async ensureClient() {
      await this.tab();
      try { const pong=await chrome.tabs.sendMessage(this.tabId,{type:'BH_PING'});if(pong?.ok)return pong; } catch {}
    ...
    async sendReadOnly(message) {
      await this.ensureClient();
      try { return await chrome.tabs.sendMessage(this.tabId,message); }
      catch {
        // Read-only requests are idempotent, so reinjection + one retry is safe.
        await this.ensureClient();
        return chrome.tabs.sendMessage(this.tabId,message);
      }
    }
    async sendMutationOnce(message) {
      await this.ensureClient();
      return chrome.tabs.sendMessage(this.tabId,message);
    }
    async inspect(options={}) {
      const result=await this.sendReadOnly({type:'BH_INSPECT',max:options.max ?? 200});
      if(!result?.ok)throw new Error(result?.error||'INSPECT_FAILED');
      return result;
    }
    async read(options={}) {
      const result=await this.sendReadOnly({type:'BH_READ',maxChars:options.maxChars ?? 16000,tail:!!options.tail});
    ...
    async act(action) {
      ...
      try {
        const result=await this.sendMutationOnce({type:'BH_ACT',action});
        if(!result?.ok&&!result?.recoverable)throw new Error(result?.error||'ACTION_FAILED');
        return result;
      } catch(err) {
        const after=await this.tab().catch(()=>null);
        if(after&&cleanComparableUrl(after.url)!==cleanComparableUrl(before.url)){
          // The mutation may already have caused navigation. Never resend it blindly.
          return {ok:true,verified:false,navigation:true,evidence:{type:'navigation_after_delivery_loss',url:after.url}};
        }
        throw err;
      }
    }
    ...
    async findElement(spec={},options={}) {
      ...
      if(elements.length>1&&options.allowMultiple!==true)throw new Error(`Too many elements found (${elements.length}): ${JSON.stringify(spec)}`);
      return elements[0];
    }
    ...
    async waitFor(condition,options={}) {
      ...
      const timeoutMs=Number(options.timeoutMs ?? 10000),retryMs=Number(options.retryMs ?? 150),started=Date.now();
      let lastError=null;
      while(Date.now()-started<timeoutMs){
        try{const value=await condition(this);if(value)return value;}catch(err){lastError=err;}
        await sleep(retryMs);
      }
    ...
    async waitForElement(spec={},options={}) {
      return this.waitFor(async driver=>{const list=await driver.findElements(spec,{timeoutMs:0,max:options.max??400});return list[0]||false;},options);
    }
    async waitForText(text,options={}) {
      const wanted=norm(text);
      return this.waitFor(async driver=>{const page=await driver.read({maxChars:options.maxChars??50000,tail:!!options.tail});return norm(page.text).includes(wanted)?page:false;},options);
    }
    async navigate(url,options={}) {
      const before=await this.tab();const target=new URL(String(url),before.url).href;
      await chrome.tabs.update(this.tabId,{url:target});
      await this.waitFor(async()=>{const tab=await this.tab();return tab.status==='complete'?tab:false;},{timeoutMs:Number(options.timeoutMs??25000),retryMs:200});
      await this.ensureClient();return this.tab();
    }
  }

  function cleanComparableUrl(url){try{const u=new URL(url);return `${u.origin}${u.pathname}${u.search}`;}catch{return String(url||'');}}
  const api={Driver,ElementProxy,matchesDescriptor,sleep};
  if(typeof module!=='undefined'&&module.exports)module.exports=api;else root.BH_CORE=api;
