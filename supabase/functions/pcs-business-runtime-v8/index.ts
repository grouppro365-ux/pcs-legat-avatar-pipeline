...
function rentalRangeFrom(s=''){const t=String(s||'').toLowerCase();const names=Object.keys(RENT_MONTHS).join('|');const m=t.match(new RegExp(`(?:с|от|from)?\\s*(\\d{1,2})\\s*(?:по|до|[-–—]|to)\\s*(\\d{1,2})\\s*(${names})(?:\\s+(\\d{4}))?`,'iu'));if(!m)return null;let y=m[4]?Number(m[4]):new Date().getFullYear(),month=RENT_MONTHS[m[3].toLowerCase()];let start=new Date(y,month,Number(m[1]),12),end=new Date(y,month,Number(m[2]),12);if(!m[4]&&end.getTime()<Date.now()-7*86400000){start=new Date(y+1,month,Number(m[1]),12);end=new Date(y+1,month,Number(m[2]),12)}if(start.getDate()!==Number(m[1])||end.getDate()!==Number(m[2]))return null;const days=rentalDays(dateIso(start),dateIso(end));if(!days)return null;return{start:dateIso(start),end:dateIso(end),duration:rentalDurationLabel(days)}}
