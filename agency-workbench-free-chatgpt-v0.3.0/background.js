const STORE = {
  CHAT: 'agency.chatBinding.v3',
  TARGET: 'agency.targetBinding.v3'
};
...
async function bindChat(tabId) {
  ...
  if (!tab || !u || u.hostname !== 'chatgpt.com') {
    throw new Error('Выбранная вкладка не является chatgpt.com');
  }
  const health = await injectAndPing(tab.id, 'chat');
  if (!health.ok) throw new Error(health.error);
  const binding = {
    tabId: tab.id,
    conversationPath: chatConversationPath(tab.url),
...
async function bindTarget(tabId) {
  ...
  const u = parseHttpUrl(tab?.url);
  if (!tab || !u || u.hostname === 'chatgpt.com') {
    throw new Error('Выберите обычную рабочую вкладку сайта');
  }
...
async function validateChatBinding() {
  ...
  const u = parseHttpUrl(tab?.url);
  if (!tab || !u || u.hostname !== 'chatgpt.com') {
    throw new Error('Привязанная вкладка ChatGPT закрыта или больше не ChatGPT');
  }
  const currentPath = chatConversationPath(tab.url);
  if (binding.conversationPath && currentPath !== binding.conversationPath) {
    throw new Error('В привязанной вкладке открыт другой разговор ChatGPT. Перепривяжите вкладку.');
  }
  const health = await injectAndPing(tab.id, 'chat');
  if (!health.ok) throw new Error(health.error);
...
async function validateTargetBinding() {
  ...
  if (u.origin !== binding.origin) {
    throw new Error(`Рабочая вкладка ушла на другой сайт: ${u.origin}. Перепривяжите её явно.`);
  }
  const health = await injectAndPing(tab.id, 'page');
  if (!health.ok) throw new Error(health.error);
  return { binding, tab, health };
...
async function diagnose() {
  ...
  try {
    ...
    result.target = { ok: false, error: e.message };
  }
  return result;
}
...
async function sendToChatGPT(prompt, requestId, timeoutMs = 180000) {
  const x = await validateChatBinding();
  const response = await sendTabMessage(x.tab.id, {
    type: 'AGENCY_CHAT_SEND',
    prompt,
    requestId,
    timeoutMs
  });
...
async function executeTarget(action) {
  const x = await validateTargetBinding();
  const response = await sendTabMessage(x.tab.id, {
    type: 'AGENCY_PAGE_EXECUTE',
    action
  });
  if (!response?.ok && !response?.confirmationRequired) {
    throw new Error(response?.error || 'Действие на странице не выполнено');
  }

  const after = await getTab(x.tab.id);
  const afterUrl = parseHttpUrl(after?.url);
  if (afterUrl && afterUrl.origin !== x.binding.origin) {
    throw new Error(`После действия вкладка перешла на другой origin: ${afterUrl.origin}. Остановлено.`);
  }
  return response;
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    try {
      switch (message?.type) {
        case 'AGENCY_LIST_TABS':
          sendResponse({ ok: true, tabs: await listTabs() });
          break;
        case 'AGENCY_BIND_CHAT':
          sendResponse({ ok: true, ...(await bindChat(message.tabId)) });
          break;
        case 'AGENCY_BIND_TARGET':
          sendResponse({ ok: true, ...(await bindTarget(message.tabId)) });
          break;
        case 'AGENCY_GET_BINDINGS':
          sendResponse({
            ok: true,
            chat: await getStored(STORE.CHAT),
            target: await getStored(STORE.TARGET)
          });
          break;
        case 'AGENCY_DIAGNOSE':
          sendResponse({ ok: true, diagnosis: await diagnose() });
          break;
        case 'AGENCY_CHAT_SEND_REQUEST':
          sendResponse({ ok: true, result: await sendToChatGPT(message.prompt, message.requestId, message.timeoutMs) });
          break;
        case 'AGENCY_TARGET_SCAN_REQUEST':
          sendResponse({ ok: true, scan: await scanTarget() });
          break;
        case 'AGENCY_TARGET_EXECUTE_REQUEST':
          sendResponse({ ok: true, result: await executeTarget(message.action) });
          break;
        default:
