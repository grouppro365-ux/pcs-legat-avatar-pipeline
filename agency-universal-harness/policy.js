(() => {
  const ALLOWED = new Set(['click','fill','select','navigate','assert','wait','check','uncheck','hover','scroll']);
  ...
  function validateAction(action, ctx = {}) {
    ...
    if (action.type === 'navigate') {
      try {
        ...
        if (!/^https?:$/.test(target.protocol)) return {ok:false,code:'UNSUPPORTED_URL_SCHEME'};
      } catch { return {ok:false,code:'BAD_URL'}; }
    }

    const label = `${action.target?.name || ''} ${action.target?.text || ''} ${action.target?.label || ''} ${action.target?.hints?.placeholder || ''}`.trim();
  ...
  function abstractAction(action) {
    ...
    if (action.target) {
      safe.target = {
        ...
        hints:{
          id:action.target.hints?.id || '',
          name:action.target.hints?.name || '',
          testid:action.target.hints?.testid || '',
          aria:action.target.hints?.aria || '',
          placeholder:action.target.hints?.placeholder || '',
          hrefPath:normalizeUrl(action.target.hints?.hrefPath || '')
        }
      };
    }
    if (action.type === 'navigate') safe.path = normalizeUrl(action.url);
    if (action.type === 'assert') {
      if (action.equals != null) safe.equals='[VALUE]';
      if (action.includes != null) safe.includes='[VALUE]';
    }
    return safe;
  }
