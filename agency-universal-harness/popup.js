...
function renderBind(el, ok, text) {
  el.innerHTML = `<span class="dot ${ok?'ok':''}"></span>${esc(text)}`;
}
...
function render() {
  ...
  if (run?.result) { $('result').classList.remove('hidden'); $('result').textContent = run.result; }
  else $('result').classList.add('hidden');
  if (run?.evidence) { $('evidence').classList.remove('hidden'); $('evidence').textContent = `Проверка: ${run.evidence}`; }
  else $('evidence').classList.add('hidden');
  ...
  if (pending) {
    ...
    $('confirmText').textContent = `${a.type || 'action'} → ${a.target?.name || a.target?.label || a.target?.text || a.target?.ref || ''}`;
...
async function refresh() {
  try {
    const res = await call('AUH_GET_STATE');
    if (res?.ok) { state = res.state; render(); }
  } catch (err) {
    $('runStatus').textContent = `Ошибка: ${err.message || err}`;
  }
}

function showError(err) {
  $('runStatus').className = 'status blocked';
  $('runStatus').textContent = `Ошибка: ${err?.message || err}`;
}

$('bindChat').onclick = async () => {
  try { const r=await call('AUH_BIND_CHAT'); if(!r?.ok) throw new Error(r?.error); state=r.state; render(); } catch(e){showError(e);}
};
$('bindTarget').onclick = async () => {
  try { const r=await call('AUH_BIND_TARGET'); if(!r?.ok) throw new Error(r?.error); state=r.state; render(); } catch(e){showError(e);}
};
$('start').onclick = async () => {
  try {
    const task=$('task').value.trim();
    const r=await call('AUH_START_TASK',{task}); if(!r?.ok) throw new Error(r?.error);
...
$('approve').onclick = async () => {
  try {
    const token=$('approve').dataset.token;
    const r=await call('AUH_APPROVE_PENDING',{token}); if(!r?.ok) throw new Error(r?.error);
    await refresh();
  } catch(e){showError(e);}
};
