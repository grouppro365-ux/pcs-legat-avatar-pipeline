(() => {
  ...
  function scan() {
    ...
    const nodes = AUH_LOCATOR.interactiveNodes(360);
    const elements = nodes.map(el => {
      const d = AUH_LOCATOR.descriptor(el);
      const ref = refFor(el);
      const label = `${d.name} ${d.hints?.placeholder || ''}`.trim();
      return {ref,...d,sensitive:SECRET_INPUT.test(label)||String(el.type||'').toLowerCase()==='password'};
    });
    return {epoch,url:safeUrl(),title:cleanText(document.title,240),visibleText:redact(cleanText(document.body?.innerText||'',8000)),elements,fingerprint:pageFingerprint()};
  }
  ...
  async function stableValue(el, waitMs=220) {
  ...
  function waitForMutation(before, timeoutMs=3500) {
    return new Promise(resolve=>{
      ...
      let done=false;
      const finish=changed=>{if(done)return;done=true;observer.disconnect();clearTimeout(timer);resolve({changed,fingerprint:pageFingerprint()});};
      const observer=new MutationObserver(()=>{if(pageFingerprint()!==before)finish(true);});
  ...
  async function act(action) {
    ...
    if(action.type==='scroll'&&!action.target?.ref&&!action.target?.name&&!action.target?.text){
      const y=Number(action.y??action.deltaY??700);window.scrollBy({top:Number.isFinite(y)?y:700,behavior:'auto'});
    ...
    if(action.type==='fill'){
      ...
      const actual=await stableValue(el);
      const ok=actual===String(action.value??'');
      return{ok,verified:ok,...recovery,error:ok?undefined:'FIELD_VALUE_REVERTED',recoverable:!ok,evidence:{type:'field_value',actual:ok?'[MATCH]':cleanText(actual,160)}};
    }
    ...
    if(action.type==='select'){
      ...
      return{ok,verified:ok,...recovery,error:ok?undefined:'SELECT_VALUE_REVERTED',recoverable:!ok,evidence:{type:'select_value',actual:ok?'[MATCH]':cleanText(el.value,120)}};
    }

    if(action.type==='check'||action.type==='uncheck'){
      if(!('checked' in el))return{ok:false,error:'NOT_CHECKABLE',recoverable:true};
      const desired=action.type==='check';if(!!el.checked!==desired)el.click();await new Promise(r=>setTimeout(r,160));
      const ok=!!el.checked===desired;return{ok,verified:ok,...recovery,evidence:{type:'checked',value:!!el.checked}};
    }

    if(action.type==='assert'){
      const actual=valueOf(el)||cleanText(el.innerText||el.textContent,3000);let ok=false;
    ...
    if(action.type==='hover'){
      el.scrollIntoView({block:'center',inline:'center'});el.dispatchEvent(new MouseEvent('mouseover',{bubbles:true}));el.dispatchEvent(new MouseEvent('mouseenter',{bubbles:true}));
      return{ok:true,verified:true,...recovery,evidence:{type:'hover',target:AUH_LOCATOR.nameOf(el)}};
    }

    if(action.type==='scroll'){
      el.scrollIntoView({block:action.block||'center',inline:'nearest'});return{ok:true,verified:true,...recovery,evidence:{type:'scroll_to',target:AUH_LOCATOR.nameOf(el)}};
    }

    if(action.type==='click'){
      if(el.tagName==='A'&&el.href){try{const next=new URL(el.href,location.href);if(!/^https?:$/.test(next.protocol))return{ok:false,error:'UNSUPPORTED_URL_SCHEME'};}catch{}}
      const form=el.closest('form');
      if(form?.action){try{const next=new URL(form.action,location.href);if(next.origin!==location.origin)return{ok:false,error:'CROSS_ORIGIN_FORM_BLOCKED'};}catch{}}
      const before=pageFingerprint();const beforeUrl=safeUrl();el.scrollIntoView({block:'center',inline:'center'});el.click();const change=await waitForMutation(before);const afterUrl=safeUrl();const navigation=afterUrl!==beforeUrl;
      return{ok:true,verified:change.changed||navigation,navigation,expectedUrl:navigation?afterUrl:undefined,expectedOrigin:navigation?location.origin:undefined,...recovery,evidence:{type:'state_change',changed:change.changed,navigation}};
    }
  ...
  chrome.runtime.onMessage.addListener((msg,_sender,sendResponse)=>{
    ...
    if(msg?.type==='AUH_PAGE_ACT'){Promise.resolve(act(msg.action)).then(sendResponse).catch(err=>sendResponse({ok:false,error:String(err?.message||err)}));return true;}
  });
})();
