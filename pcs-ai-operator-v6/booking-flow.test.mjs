...
assert.match(ops,/Number\(x\.daily_price\)>0/,'cars with an explicit daily tariff must stay selectable when a legacy period field is absent');
assert.match(ops,/x\.daily_price\?\?x\.final_price/,'the booking price must prefer the explicit daily tariff');
