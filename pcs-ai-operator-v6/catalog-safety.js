...
window.pricingManager=async function(id){
  ...
  if(base&&!document.querySelector('#basePricePeriod')){
    base.insertAdjacentHTML('afterend',`<div class="field"><label>Базовая цена указана за</label><select id="basePricePeriod"><option value="day" ${x.base_price_period==='day'?'selected':''}>День</option><option value="week" ${x.base_price_period==='week'?'selected':''}>Неделю</option><option value="month" ${x.base_price_period==='month'?'selected':''}>Месяц</option></select><div class="muted">Сезонный автоматический расчёт по дням применяется только к тарифу «за день». Для недельного/месячного тарифа система не угадывает цену.</div></div>`);
  }
};
window.saveBasePrice=async function(id){try{const period=document.querySelector('#basePricePeriod')?.value||'day';await adminCall({action:'pricing',id,base_price:Number(document.querySelector('#basePrice').value),base_price_period:period,pricing_locked:document.querySelector('#pricingLocked').checked});PCS.catalog=await call('/catalog');toast('Цена и период сохранены');closeSheet();catalog()}catch(e){toast(e.message)}};
